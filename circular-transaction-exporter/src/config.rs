...
pub const DEFAULT_MAX_IN_FLIGHT: usize = 5_120;
pub const DEFAULT_CONNECT_TIMEOUT_MS: u64 = 5_000;
pub const DEFAULT_REQUEST_TIMEOUT_MS: u64 = 2_000;
/// Fixed TTL of the exact byte-level deduplication window applied to every
/// transaction before it is sent to Circular Fast. Not configurable: this is
/// a correctness safeguard against upstream retransmits/replays, not a
/// tuning knob.
pub const DEDUP_TTL: Duration = Duration::from_secs(10);
...
pub struct CircularExportConfig {
    ...
    /// Whether to export transactions verified by the native TPU sigverify
    /// hook (`--circular-forward-tpu`). Defaults to `true`.
    pub forward_tpu: bool,
    /// Whether to export transactions/bundles verified by the BAM
    /// post-sigverify hook (`--circular-forward-preconf`). Defaults to
    /// `true`.
    pub forward_preconf: bool,
    /// Whether to export classic (non-BAM) Jito block-engine bundles
    /// verified by `BundleSigverifyStage`
    /// (`--circular-forward-jito-bundle`). Defaults to `true`.
    pub forward_jito_bundle: bool,
}
...
impl Default for CircularExportConfig {
    fn default() -> Self {
        Self {
            ...
            forward_tpu: true,
            forward_preconf: true,
            forward_jito_bundle: true,
        }
    }
}

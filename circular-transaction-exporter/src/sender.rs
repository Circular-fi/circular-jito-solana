...
impl CircularExportSender {
    ...
    pub fn new_for_tests(queue_capacity: usize) -> (Self, crossbeam_channel::Receiver<ExportItem>) {
        ...
        (Self::new(sender, Arc::default()), receiver)
    }
    ...
    pub fn new_for_tests_with_mode(
        ...
        let (sender, receiver) = crossbeam_channel::bounded(queue_capacity);
        let mut sender = Self::new(sender, Arc::default());
        sender.mode = mode;
        (sender, receiver)
    }
    ...
    /// builds the strategy is selected by [`HookMode`]. Vote transactions are
    /// never exported: when `is_tpu_vote` is set, this is a no-op.
    #[inline]
    pub fn export_verified(&self, batches: &Arc<Vec<PacketBatch>>, is_tpu_vote: bool) {
        ...
        #[cfg(feature = "dev-context-only-utils")]
        {
            match self.mode {
                ...
                HookMode::Arc => self.try_send_shared(batches.clone(), false),
            }
        }
        #[cfg(not(feature = "dev-context-only-utils"))]
        {
            self.try_send_shared(batches.clone(), false);
        }
    }
    ...
    /// `is_bundle` selects the memo tag the exporter attaches to every
    /// transaction in this batch (currently BAM-specific: single transaction
    /// vs. atomic multi-transaction batch, see
    /// [`export_bam_shared`](Self::export_bam_shared)).
    ///
    ...
    /// scheduled for it.
    #[inline]
    pub fn try_send_shared(&self, batches: Arc<Vec<PacketBatch>>, is_bundle: bool) {

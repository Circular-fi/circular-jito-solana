use {
    crate::{
        ...
        event::{ExportItem, TransactionSource},
...
impl CircularTransactionExporter {
    ...
    pub fn spawn(
        ...
        let sender = CircularExportSender::new(batch_sender, metrics.clone());
...
async fn run_exporter(
    ...
    while let Some(item) = bridge_receiver.recv().await {
        match item {
            ...
            // filtering happen here, off the sigverify hot path. Currently
            // only fed by the BAM hook; `is_bundle` selects the memo.
            ExportItem::Shared(shared) => {
                let source = if shared.is_bundle {
                    TransactionSource::BamBundle
                } else {
                    TransactionSource::Bam
                };

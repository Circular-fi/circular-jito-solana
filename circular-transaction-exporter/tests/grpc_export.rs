...
/// With a non-zero dedup TTL, sending the exact same wire bytes twice within
/// the window must only reach Fast once; the second copy is dropped and
/// counted.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn duplicate_transaction_within_ttl_is_sent_once() {
    let (addr, mut events) = spawn_server(false);
    let (sender, exporter) = CircularTransactionExporter::spawn_with_dedup_ttl(
        test_config(addr),
        "identity".to_string(),
    ...
    sender.try_send(packet_batch(&[(b"same-bytes", TransactionSource::Tpu)]));
    sender.try_send(packet_batch(&[(b"same-bytes", TransactionSource::Tpu)]));
    // A distinct payload proves the exporter is still alive and processing,
    // rather than the second duplicate merely being slow to arrive.
    sender.try_send(packet_batch(&[(b"other-bytes", TransactionSource::Tpu)]));

    // Each submission is dispatched as an independent concurrent gRPC call,
    // so only the *set* of delivered payloads (not their arrival order) is
    // guaranteed.
    let mut received = HashSet::new();
    received.insert(next_event(&mut events).await.transaction);
    received.insert(next_event(&mut events).await.transaction);
    let expected: HashSet<Vec<u8>> = [b"same-bytes".to_vec(), b"other-bytes".to_vec()]
        .into_iter()
        .collect();
    assert_eq!(received, expected);
    ...
    assert_eq!(metrics.dropped_duplicates.load(Ordering::Relaxed), 1);
}

/// With deduplication disabled (`Duration::ZERO`), the exact same wire bytes
/// sent twice must both reach Fast — this is the behavior every other test
/// in this file relies on.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn duplicate_transaction_with_dedup_disabled_is_sent_twice() {
    let (addr, mut events) = spawn_server(false);
    let (sender, exporter) = CircularTransactionExporter::spawn_with_dedup_ttl(
        test_config(addr),
        "identity".to_string(),
    ...
    assert_eq!(first.transaction, b"same-bytes");
    let second = next_event(&mut events).await;
    assert_eq!(second.transaction, b"same-bytes");

    drop(sender);
    join_exporter(exporter).await;

    assert_eq!(metrics.dropped_duplicates.load(Ordering::Relaxed), 0);
}

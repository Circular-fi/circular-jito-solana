...
impl Tpu {
    ...
    pub fn new_with_client(
        ...
        // Exporter runs on its own thread; sigverify (both native TPU and
        // BAM) only ever try_sends. Boot log is emitted here (solana_core)
        // so it passes the default `solana=info,agave=info` filter; the
        // circExporter crate's own info! lines are otherwise invisible.
        // Built before `SigVerifyStage` so the native TPU hook can be wired
        // in from the start.
